...
from .team_mapping import ESPN_TEAM_IDS, espn_id_for, from_sport_suite_abbrev
...
async def get_team_roster(abbrev: str) -> list[RosterPlayer]:
    ...
    for athlete in athletes:
        players.append(RosterPlayer(
            ...
            age=athlete.get("age"),
            experience=str(athlete.get("experience", {}).get("years", "R")) if athlete.get("experience") else "R",
            college=athlete.get("college", {}).get("name", "") if athlete.get("college") else "",
            headshot_url=athlete.get("headshot", {}).get("href", "") if athlete.get("headshot") else "",
        ))
...
async def get_team_schedule(abbrev: str, session: AsyncSession) -> list[TeamScheduleGame]:
    ...
    for g in games:
        ...
        schedule.append(TeamScheduleGame(
            game_id=g.id,
