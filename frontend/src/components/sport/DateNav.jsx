    ...
    <div className="flex flex-col gap-8 mb-12">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="h-12 w-12 rounded-2xl bg-white/5 border border-white/10 text-white/40 shadow-2xl flex items-center justify-center rim-light">
          ...
          {dates.map((date) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm font-black uppercase tracking-widest mb-1 ${isActive ? 'opacity-60' : 'opacity-40'}`}>{formatDate(date, 'weekday')}</span>
                <span className="text-xl font-black tabular-nums tracking-tighter">{formatDate(date, 'day')}</span>
                <span className={`text-[13px] font-black uppercase tracking-[0.2em] mt-1 ${isActive ? 'opacity-40' : 'opacity-20'}`}>{formatDate(date, 'month')}</span>
              </Link>
            );
          })}
        </div>
